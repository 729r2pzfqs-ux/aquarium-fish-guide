...
with open(BASE_DIR / 'data' / 'translations_fr.json', encoding='utf-8') as _f:
...
def locale_names(fish_dir):
    """fish id -> the name the existing fr pages use, for link/alt text."""
    table = {}
    base = Path(fish_dir)
    if base.is_dir():
...
LOCALE_NAMES = locale_names(FR_FISH_DIR)
